...
def cmd_srt(args):
    ...
    print_info(f"Rodando: {' '.join(cmd)}")

    with Spinner("Transcrevendo com Whisper..."):
        result = subprocess.run(cmd, capture_output=True, text=True)

    if result.returncode != 0:
        print_error("Whisper retornou um erro:")
        print(result.stderr)
        sys.exit(1)
    ...
    print_success(f"SRT gerado em: \033[1m{srt_path}\033[0m")
    return srt_path